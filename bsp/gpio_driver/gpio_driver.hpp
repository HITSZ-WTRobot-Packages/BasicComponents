 ...
 * 推荐使用此类型替代 GPIO_t。
 */
struct GpioPinOutput : GpioPin
{
    void set() const { write(GPIO_PIN_SET); }

    void reset() const { write(GPIO_PIN_RESET); }

    void toggle() const { HAL_GPIO_TogglePin(port, pin); }
};

struct GpioPinInput : GpioPin
{
    GpioPinInput(GPIO_TypeDef* port, const uint16_t pin, void* user_data = nullptr)
    {
        this->port      = port;
        this->pin       = pin;
    ...
    [[nodiscard]] bool isSet() const { return read() == GPIO_PIN_SET; }
    [[nodiscard]] bool isReset() const { return read() == GPIO_PIN_RESET; }
};
...
using ExtiCallback = void (*)(const GpioPinInput* gpio, uint32_t counter);

/// 注册某个 GPIO 的 EXTI 回调。回调上下文从 gpio->user_data 读取。
void RegisterExtiCallback(const GpioPinInput* gpio, ExtiCallback callback);
